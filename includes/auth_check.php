...
function check_auth($required_role = null) {
    ...
    if (!isset($_SESSION['logged_in']) || $_SESSION['logged_in'] !== true) {
        header('Location: /SAO/index.php?error=3'); // Not logged in
        exit;
    }

    // If a specific role is required, check for it
    if ($required_role !== null && (!isset($_SESSION['role']) || $_SESSION['role'] !== $required_role)) {
        header('Location: /SAO/index.php?error=4'); // Unauthorized role
        exit;
    }
    ...
    if (isset($_SESSION['last_activity']) && (time() - $_SESSION['last_activity'] > $session_lifetime)) {
        session_unset();
        session_destroy();
        header('Location: /SAO/index.php?error=5'); // Session expired
        exit;
    }
